...
const Cashback: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.recipientUsername || !formData.currencyCode || !formData.amount) {
    ...
    try {
      ...
      alert(`Transfer successful! Sent ${formData.amount} ${formData.currencyCode} to ${formData.recipientUsername}`);
  ...
  const selectedWallet = cards.find(card => card.currency === formData.currencyCode);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Recipient Username */}
          <div>
            <label htmlFor="recipientUsername" className="block text-sm font-medium text-gray-700 mb-2">
              Recipient Username
            </label>
            <input
              type="text"
          ...
          <button
            type="submit"
            disabled={loading || !selectedWallet || parseFloat(formData.amount) > selectedWallet.balance}
          ...
          {selectedWallet && parseFloat(formData.amount) > selectedWallet.balance && (
            <p className="text-red-600 text-sm text-center">
              Insufficient balance. You only have {selectedWallet.balance} {selectedWallet.currency}
            </p>
          )}
        </form>
      </div>
